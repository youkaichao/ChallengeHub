  ...
  <div>
    ...
    <el-table :data="contests">
      ...
      <el-table-column label="报名开始" prop="enrollStart">
        <template slot-scope="scope">{{ isoToHumanReadable(scope.row.enrollStart) }}</template>
      </el-table-column>
      <el-table-column label="报名结束" prop="enrollEnd">
        <template slot-scope="scope">{{ isoToHumanReadable(scope.row.enrollEnd) }}</template>
      </el-table-column>
    ...
    <el-button
      ...
      @click="$router.push('/contest/create')"
    >创建比赛</el-button>
  </div>
</template>
