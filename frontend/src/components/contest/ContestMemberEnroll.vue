...
export default {
  ...
  created: async function() {
    let contestId = this.$route.params.contest_id
  ...
  methods: {
    enrollSubmit: async function() {
      ...
      let response = await this.$http.post(`/apiv2/contests/${contestId}/groups/${teamId}/invitation`, postData)
      if (response.body.code !== 0) {
